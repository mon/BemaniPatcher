...
(function (window, document) {
	...
	// shorthand functions
	var createElementClass = function (
		elName,
		className,
		textContent,
		innerHTML
	...
	class StandardPatch {
		...
		createUI(parent) {
			...
			if (this.danger) {
				patch.appendChild(
					createElementClass('div', 'danger tooltip', this.danger)
				);
			}
		...
		validatePatch(file) {
			...
			if (status === 'on') {
				...
				console.log(`It's offset is ${this.patches[0].offset}`); */
			} else {
				return (
					'"' +
					this.name +
					'" is neither on nor off! Have you got the right file?'
				);
			}
		}
	...
	class DynamicPatch {
		...
		validatePatch(file) {
			var status =
				this.mode === 'all' ? this.checkPatchAll(file) : this.checkPatch(file);
				...
				if (Array.isArray(patch.offset)) {
					for (const offset of patch.offset) {
						if (this.target === 'string') {
							...
							patch.on = patch.on.map((patch, idx) =>
								patch === 'XX' ? file[offset + idx] : patch
							);
							patch.off = patch.off.map((patch, idx) =>
								patch === 'XX' ? file[offset + idx] : patch
							);
							replace(file, offset, featureOn ? patch.on : patch.off);
						}
					}
				} else {
					if (this.target === 'string') {
						...
						patch.on = patch.on.map((patch, idx) =>
							patch === 'XX' ? file[patch.offset + idx] : patch
						);
						patch.off = patch.off.map((patch, idx) =>
							patch === 'XX' ? file[patch.offset + idx] : patch
						);
						replace(file, patch.offset, featureOn ? patch.on : patch.off);
					}
				}
			}
		}
						...
						if (this.target === 'string') {
							listUi.appendChild(
								createElementClass(
									...
									null,
									'0x' +
										offOffset.toString(16) +
										' <b>' +
										patch.off +
										'</b> will be replaced with <b>' +
										patch.on +
										'</b>'
								)
							);
	...
	class UnionPatch {
		...
		createUI(parent) {
			...
			container.appendChild(
				createElementClass('span', 'patch-union-title', this.name + ':')
			);
			if (this.tooltip) {
				container.appendChild(
					createElementClass('div', 'tooltip', this.tooltip)
				);
			}
			if (this.danger) {
				container.appendChild(
			...
			for (var i = 0; i < this.patches.length; i++) {
				...
				if (patch.tooltip) {
					patchDiv.appendChild(
						createElementClass('div', 'tooltip', patch.tooltip)
					);
				}
				if (patch.danger) {
					patchDiv.appendChild(
						createElementClass('div', 'danger tooltip', patch.danger)
					);
				}
		...
		validatePatch(file) {
			...
			return (
				'"' +
				this.name +
				'" doesn\'t have a valid patch! Have you got the right file?'
			);
		}
	...
	class HexPatch {
		...
		createUI(parent) {
			...
			patchDiv.appendChild(
				createElementClass('div', 'tooltip', 'Value ' + bytesToHex(this.off))
			);
			container.appendChild(patchDiv);
			...
			patchDiv.appendChild(radio);
			patchDiv.appendChild(
				createLabel('Custom ' + this.off.length + '-byte hex value: ', id)
			);
			...
			patchDiv.appendChild(
				createElementClass(
					'div',
					'danger tooltip',
					'Invalid values will not be applied.'
				)
			);
			container.appendChild(patchDiv);
		...
		updateUI(file) {
			...
			this.valueHex.value = bytesToHex(
				file.slice(this.offset, this.offset + this.off.length)
			);
	...
	class PatchContainer {
		...
		createUI() {
			...
			container.appendChild(
				createElementClass('h4', null, 'Supported Versions:')
			);
			container.appendChild(
				createElementClass('h5', null, 'Click name to preview patches')
			);
			...
			/* topMessage.innerText =
				'Place a DLL here to patch it,or download a JSON and patch it through SpiceTools'; */
			topMessage.innerText =
				'Apply patches by providing a .dll here, or download a JSON file and use SpiceTools instead.';
			...
			const appendToggles = () => {
				toggleOptions.forEach((option, index) => {
					...
					toggle.type = 'radio';
					toggle.id = 'spiceToggle';
					toggle.className = `spice-toggle${index + 1}`;
					toggle.name = 'spiceToggle';
					if (option === toggleOptions[0]) {
						toggle.checked = true; // Always checks the first option by default
					}
					const toggleLabel = document.createElement('label');
					toggleLabel.for = 'spiceToggle';
			...
			reader.onload = function (e) {
				...
				for (var i = 0; i < self.patchers.length; i++) {
					...
					if (patcher.validatePatches()) {
						...
						self.matchSuccessText[i].textContent =
							' ' + valid + ' of ' + valid + ' patches matched (100%) ';
					}
				}
